...
class ZeroShotImprisonmentPredictor(BaseImprisonmentPredictor):
...
"""
    ...
    def predict(
        ...
        """
        prompt = self.build_zero_shot_prompt(fact, defendants, charge_dict=charge_dict)
    ...
    def build_zero_shot_prompt(
        fact: str, defendants: list[str], charge_dict: dict[str, list[str]]
    ) -> str:
        """
        ...
        Args:
            ...
            charge_dict (dict[str, list[str]]): Dictionary containing the predicted charge details.

        Returns:
            str: The constructed prompt string.
        """
        prompt = f"""你是一个中华人民共和国的法官，你现在需要根据检察院对多名被告的指控和罪名，依据中华人民共和国刑法，判断每个被告每个罪名的量刑，量刑以月为单位。进行判决的时候，你应该注意：
1. 你要完整、仔细的阅读检察官的指控，你的量刑必须严格的按照检察官的指控事实作出，不能脱离事实，也不必怀疑检察官的指控
2. 我们已经确定了每个被告的罪名，你需要根据这些罪名来进行量刑，轻易不要怀疑我们确定的罪名，如果确实认为不是该罪名，则将量刑设为0，但不要增加你认为正确的罪名，也不要删除我们确定的罪名
3. 每个罪名的量刑必须在中华人民共和国刑法的规定范围内，必须严格的按照法律条款来量刑
4. 你需要给出每个罪名的量刑，量刑以月为单位，量刑需要是整数，注意你需要给出完整的判决，包括有期徒刑、罚金等等，但最终的总结中你只需要给出有期徒刑的量刑，其他的刑罚不必考虑，如果不必判处有期徒刑（这是很可能的），则量刑为0
5. 特别注意被告犯下罪行的时间，如果对应的法律条款后续有修改，要遵从从旧兼从轻的原则
6. 不同被告可能会有不同的罪名，一个罪行不一定所有被告都会涉及，同时即使同一个罪名不同的被告也可能会有不同的量刑，你需要逐一分析，不能一概而论
以下是检察官的指控：
{fact}
以下是被告的名单：
{', '.join(defendants)}
...
{{"<第一个被告姓名>": [{{"standard_accusation": "第一个被告第一个罪名", "imprisonment": <对应的刑期，使用整数>}}, {{"standard_accusation": "第一个被告的第二个罪名", "imprisonment": <对应的刑期，使用整数>}}]}}, {{"<第二个被告姓名>": [{{"standard_accusation": "第二个被告的第一个罪名", "imprisonment": "<对应的刑期，使用整数>"}}]}}
"""
        return prompt
